...
int maildirWrite(char *maildir, BUFFER *message, int create)
{
	...
	for (count = 0;; count++) {
		...
		snprintf(basename, MAX_BASENAME, "%lu.%u_%lu.%s,S=%u",
			 time(NULL), getpid(), namecounter++, hostname,
			 messagesize);
		...
		if (stat(tmpname, &statbuf) == 0)
			...
			fd = open(tmpname, O_WRONLY | O_CREAT | O_EXCL,
				  S_IWUSR | S_IRUSR);
			if (fd >= 0)
...
int maildirWrite(char *maildir, BUFFER *message, int create)
{
	{
		errlog(ERRORMSG,
		       "Maildir delivery does not work with SHORTNAMES.\n");
		return -1;
	}
...
int main()
{
	...
	for (i = 0; i < count; i++) {
		maildirWriteRes =
			maildirWrite("Maildir.test_maildir", message, 1);
