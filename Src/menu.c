...
void read_folder(char command, char *foldername, char *nym)
{
	...
	BUFFER *mail, *log;
	int mailfolder =
		-1; /* -1 = unknown, 0 = no mailfolder, 1 = mailfolder */
	int num = 0;
	...
	for (;;) {
		...
		if (eof || (mailfolder && strleft(l, "From ")) ||
		    (mailfolder == 0 && from != -1 &&
		     (strileft(l, "path:") || strileft(l, "xref:") ||
		      strileft(l, "return-path")))) {
			if (num > 1)
	...
	for (;;) {
		...
		case 'd': /* delete message */
			...
			if (num > 0) {
				...
				new_folder = buf_new();
				buf_cut_out(folder, deleted_message, new_folder,
					    p, q);
    ...
    mvprintw(0, COLS - sizeof(COPYRIGHT), COPYRIGHT);
#endif
		for (space = 0; space < (COLS - 10 - sizeof(VERSION));
		     space++) {
			printw(" ");
		};
		standend();
		...
		if (c != ERR) {
			...
			case 'd':
				...
				if (mix_encrypt(MSG_NULL, NULL, NULL, 1,
						chainlist) != 0) {
					...
					for (n = 0;
					     buf_getline(chainlist, line) == 0;
					     n++)
						;
					if (n > 1)
						mix_status("Done (%d packets).",
							   n);
					else
						mix_status("Chain: %s",
							   chainlist->data);
...
redraw:
	...
	if (*d != MSG_NULL)
		mvprintw(6, 0, "d)estination: %s",
			 *d == MSG_MAIL ? mdest : pdest);
	if (psub && *d == MSG_POST)
	...
	for (;;) {
		...
		case 'd':
			...
			printw("d)estination: ");
			echo();
			wgetnstr(stdscr, *d == MSG_MAIL ? mdest : pdest,
				 LINELEN);
			noecho();
			goto redraw;
		case 'l':
...
/* chaintype 0=mix 1=ek 2=newnym */
{
	...
	for (i = 0; i < num; i++) {
		rlist[2 * i] = i + 1;
		rlist[2 * i + 1] =
			remailer[i + 1].info[chaintype].reliability -
	...
	for (i = 0; i < num && rlist[2 * i + 1] > -2; i++) {
		...
		sprintf(info, "%3.2f",
			remailer[rlist[2 * i]].info[chaintype].reliability /
				100.0);
	...
	for (;;) {
		...
		for (i = 0; i < chainlen; i++) {
			...
			if (chain[i])
				strcatn(newchain, remailer[chain[i]].name,
					CHAINMAX);
			else
				strcatn(newchain, "*", CHAINMAX);
		}
		if (chainlen > 0) {
			ok = 1;
			middlemanlast =
				remailer[chain[chainlen - 1]].flags.middle;
			if (post && !remailer[chain[chainlen - 1]].flags.post &&
			    !(chain[chainlen - 1] == 0 /*randhop*/))
				ok = 0;
		} else
			ok = 1;
		...
		if (!nymserv) {
			...
			mvprintw(LINES - 4, 58, "(reliability: %s)",
				 reliability);
		}
