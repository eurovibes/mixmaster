...
int stats(BUFFER *b)
{
   ...
   */
	...
	for (i = 0; i < 24; i++)
		msgd[0][i] = msgd[1][i] = msgd[2][i] = msgd[3][i] = msgd[4][i] =
			msgd[5][i] = msgd[6][i] = poold[0][i] = poold[1][i] = 0;
	for (i = 0; i < 80; i++)
		msg[0][i] = msg[1][i] = msg[2][i] = msg[3][i] = msg[4][i] =
			msg[5][i] = msg[6][i] = pool[0][i] = pool[1][i] = 0;
	...
	if (s != NULL) {
		...
		while (fgets(line, sizeof(line), s) != NULL) {
			...
			case '6':
				c = '\0';
				assigned = sscanf(line, "%d %d %ld %c", &type,
						  &num, &then, &c);
			...
			case 'p':
				c = '\0';
				assigned = sscanf(line, "p %d %d %ld %c", &num,
						  &i, &then, &c);
				daysum = (assigned == 4 && c == 'd');
	...
	if (f != NULL) {
		while (fread(&idbuf, 1, sizeof(idlog_t), f) ==
		       sizeof(idlog_t)) {
			then = idbuf.time;
	...
	if (b != NULL) {
		...
		for (i = 23; i >= 0; i--) {
			...
			if (UNENCRYPTED)
				buf_appendf(b, "     Unencrypted:%4d",
					    msgd[0][i]);
			if (poold[0][i] > 0)
				buf_appendf(b, "  [Pool size:%4d]",
					    poold[1][i] / poold[0][i]);
#if 0
      else
...
int conf(BUFFER *out)
{
			...
			while (buf_getline(b, line) != -1) {
				if (line->length > 0 && line->data[0] != '#') {
					if (flag == 0) {
						buf_appends(
							out,
							"In addition to other remailers, this remailer also sends mail to these\n addresses directly:\n");
						flag = 1;
					}
					buf_appendf(out, "   %b\n", line);
				}
	...
	if (b != NULL) {
		...
		if (flag == 0 && NEWS[0])
			buf_appends(
				out,
				"Note that other newsgroups may be unavailable at the remailer's news server.\n");
	}
		...
		if (PGP) {
			...
			for (i = 1; i <= num; i++) {
				if (remailer[i].flags.pgp) {
					snprintf(tmpline, LINELEN,
						 "0x%08X	\"%s <%s>\"\n",
						 pgpkeyid[i], remailer[i].name,
						 remailer[i].addr);
					tmpline[LINELEN - 1] = '\0';
