...
int pgp_getkey(int mode, int algo, int *psym, int *pmdc, long *pexpires,
 ...
 */
{
	...
	while ((type = pgp_getpacket(keypacket, p1)) > 0) {
		...
		case PGP_SIG: {
			...
			if (sigver == 4) {
				sigtype = buf_getc(p1);
				if (isPGP_SIG_CERT(sigtype) ||
				    sigtype == PGP_SIG_BINDSUBKEY ||
				    sigtype == PGP_SIG_CERTREVOKE) {
					int revoked = (sigtype ==
						       PGP_SIG_CERTREVOKE),
								...
								if (expires &&
								    ((created +
								     ...
								     (created +
								      ...
								      0))) {
									if (mode ==
									    PK_ENCRYPT) { /* allow decrypt with expired subkeys, but not encrypt */
										keytype =
											-1;
									}
								}
				...
				buf_getc(
					p1); /* One-octet length of following hashed material.  MUST be 5 */
				sigtype = buf_getc(p1);
			...
			if (sigtype == PGP_SIG_KEYREVOKE) {
				...
				if (mode == PK_SIGN ||
				    mode == PK_ENCRYPT) { /* allow verify and decrypt with revokeded keys, but not sign and encrypt */
					keytype = -1;
				}
			} /* if (sigtype == PGP_SIG_KEYREVOKE) */
				...
				if (!subkeyok || subkeyok == subkeyno)
					if (mode ==
					    PK_ENCRYPT) { /* allow decrypt with revokeded subkeys, but not encrypt */
						keytype = -1;
					}
		...
		case PGP_SECKEY:
			...
			if (key) {
				...
				if (keytype == PGP_SECKEY ||
				    keytype == PGP_SECSUBKEY) {
					csstart = key->length;
	...
	if (uidd_1) {
		...
		while (uidd_1) {
			...
			if (primary <= uidd_1->primary &&
			    created <= uidd_1->created && !uidd_1->revoked) {
		...
		if (expires &&
		    ((created + expires < now) || (created + expires < 0))) {
			if (mode == PK_SIGN ||
			    mode == PK_ENCRYPT) { /* allow verify and decrypt with expired keys, but not sign and encrypt */
				keytype = -1;
			}
		}
	} /* if (uidd_1) */
	...
	if (!subkeyok && keytype == PGP_E_ELG &&
	    (mode == PK_DECRYPT || mode == PK_ENCRYPT))
		keytype =
			-1; /* no usable subkey found, one with valid binding */
		     ...
		     BUFFER *key, BUFFER *pass, time_t *created)
{
	...
	if (outtxt != NULL) {
		...
		buf_appendf(outtxt, "%s %5d%c/%02X%02X%02X%02X ",
			    ...
			    len, algoid, id->data[4], id->data[5], id->data[6],
			    id->data[7]);
...
int pgp_makepubkey(BUFFER *keypacket, BUFFER *outtxt, BUFFER *out, BUFFER *pass,
		   int keyalgo)
{
	...
	while ((type = pgp_getpacket(keypacket, p)) > 0) {
		if (type == PGP_SECSUBKEY) {
			if (pgp_makepkpacket(PGP_PUBSUBKEY, p, outtxt, out,
					     subkey, pass, &created) == -1)
				goto end;
			if (pgp_sign(pubkey, subkey, sig, NULL, pass,
				     PGP_SIG_BINDSUBKEY, 0, created, 0, seckey,
				     NULL) != -1)
				buf_cat(out, sig);
			if (outtxt)
				buf_nl(outtxt);
			...
			err = pgp_sign(pubkey, p, sig, NULL, pass, PGP_SIG_CERT,
				       1, created, 0, seckey,
				       NULL); /* maybe PGP_SIG_CERT3 ? */
			buf_cat(out, p);
			if (err == 0)
				buf_cat(out, sig);
		      ...
		      int keyalgo)
{
	...
	thisalgo =
		pgp_makepkpacket(type, p, outtxt, tmp, pubkey, pass, &created);
	if (thisalgo == -1 || (keyalgo != 0 && keyalgo != thisalgo))
		goto end;

	while ((type = pgp_getpacket(keypacket, p)) > 0) {
		if (type == PGP_SECSUBKEY || type == PGP_PUBSUBKEY) {
			if (pgp_makepkpacket(type, p, outtxt, dummy, subkey,
					     pass, &created) == -1)
				goto end;
			buf_nl(outtxt);
		} else if (type == PGP_USERID) {
...
{
	BIGNUM *a = NULL, *b = NULL, *c = NULL, *p = NULL, *g = NULL, *x = NULL;
	BN_CTX *ctx;
	BUFFER *i;
	int err = -1;
	...
	if (BN_mod_exp(c, a, x, p, ctx) == 0)
		goto end;
	if (BN_mod_inverse(a, c, p, ctx) == 0)
		goto end;
	if (BN_mod_mul(c, a, b, p, ctx) == 0)
		goto end;
	...
	in->length = RSA_padding_check_PKCS1_type_2(
		in->data, in->length, i->data, i->length, i->length + 1);
	if (in->length <= 0)
		in->length = 0;
	else
		err = 0;
...
int pgp_elgencrypt(BUFFER *in, BUFFER *key)
{
	...
	if (RSA_padding_add_PKCS1_type_2(i->data, i->length, in->data,
					 in->length) != 1)
		goto end;
	...
	if (BN_mod_exp(a, g, k, p, ctx) == 0)
		goto end;
	if (BN_mod_exp(c, y, k, p, ctx) == 0)
		goto end;
	if (BN_mod_mul(b, m, c, p, ctx) == 0)
		goto end;
