...
/* hdr = 1: mail header, hdr = 2: pasted header, hdr = 0: ignore */
{
	...
	BUFFER *cutmarks, *to, *newsgroups, *ek, *ekdes, *ekcast, *esub,
		*subject;
...
header:
	while (buf_getheader(in, field, content) == 0) {
		...
		if (bufieq(field, "test-to"))
			...
			if (type == -1)
				type = MSG_MAIL;
		} else if (bufieq(field, "anon-post-to") ||
			...
			type = MSG_NULL;
#ifdef USE_IDEA
		else if (bufieq(field, "encrypt-key") ||
			 bufieq(field, "encrypt-idea"))
			buf_set(ek, content);
#else
		else if (bufieq(field, "encrypt-key") ||
			 bufieq(field, "encrypt-idea"))
...
#endif
		else if (bufieq(field, "encrypt-des") ||
			 bufieq(field, "encrypt-3des"))
			buf_set(ekdes, content);
		else if (bufieq(field, "encrypt-cast") ||
			 bufieq(field, "encrypt-cast5"))
			buf_set(ekcast, content);
		else if (bufieq(field, "encrypt-subject"))
			buf_set(esub, content);
		else if (bufieq(field, "hash-subject") &&
			 bufieq(content, "yes"))
			hsub = 1;
		...
		} else if (bufieq(field, "max-size") ||
			   bufieq(field, "maxsize"))
			maxsize = readnum(content, 1024);
		else if (bufieq(field, "max-count") ||
			 bufieq(field, "maxcount"))
	...
	if (cutmarks->length > 0) {
		...
		while ((err = buf_getline(in, line)) != -1 &&
		       !buf_eq(line, cutmarks)) {
			buf_cat(temp, line);
			buf_nl(temp);
		}
		while (err != -1) {
	...
	if (encrypted == 1) {
		...
		if (err != -1) {
			buf_rest(
				temp,
				in); /* dangerous, but required for reply blocks */
			buf_move(in, temp);
			encrypted = 0;
			hdr = 0;
			goto header;
  ...
  if (err == -1)
	...
	if (isline(line, HDRMARK) && (hdr == 0 || hdr == 1)) {
		...
		buf_getline(in, NULL);
		for (;;) {
			if (buf_lookahead(in, line) == 0 &&
			    bufileft(line, "subject:")) {
	...
	if (maxcount >= 0) {
		...
		if (reached_maxcount(digest, maxcount)) {
			if (testto->length == 0)
				errlog(LOG,
				       "Max-Count reached - discarding message.\n");
			buf_appends(
				test,
				"Max-Count reached - discarding message.\n");
			err = -2;
...
#ifdef USE_PGP
		if (ekcast->length) {
			err = pgp_encrypt(PGP_CONVCAST | PGP_TEXT, temp, ekcast,
					  NULL, NULL, NULL, NULL);
			buf_clear(ekcast);
		}
		if (ekdes->length) {
			err = pgp_encrypt(PGP_CONV3DES | PGP_TEXT, temp, ekdes,
					  NULL, NULL, NULL, NULL);
			buf_clear(ekdes);
		}
		if (ek->length) {
			err = pgp_encrypt(PGP_CONVENTIONAL | PGP_TEXT, temp, ek,
					  NULL, NULL, NULL, NULL);
			buf_clear(ek);
		}
	...
	if (testto->length) {
		...
		if (err == 0) {
			...
			if (err == -1)
				buf_appends(
					report,
					"This remailer cannot deliver the message.\n\n");
			else {
				buf_appends(report, "Valid ");
				buf_appends(report, type == MSG_POST ?
							    "Usenet" :
							    "mail");
				...
				if (remixto->length) {
					if (remix && MIX)
						...
						buf_appends(
							report,
							"Delivery via Cypherpunk remailer: ");
			...
			buf_appends(report,
				    "The remailer message is invalid.\n\n");
			if (test->length) {
				buf_appends(report,
