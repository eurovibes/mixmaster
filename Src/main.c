...
#endif /* WIN32SERVICE */
{
	int error = 0, deflt = 1, help = 0, readmail = 0, send = -1,
	    sendpool = 0, header = 1, maint = 0, keygen = 0, verbose = 0,
	    sign = 0, encrypt = 0, redirect_mail = 0, about = 0, version = 0;
			...
			if (p[1] == '-') {
				...
				if (strieq(p, "help"))
					...
					if (statssrc->length > 0) {
						...
						deflt = 0;
						fprintf(stderr,
							"%s: No current stats source --%s\n",
							argv[0], p);
					}
				} else if (strleft(p, "lifetime") &&
					   p[strlen("lifetime")] == '=') {
					lifeindays = strtol(
						p + strlen("lifetime") + 1,
						NULL, 10);
					if (lifeindays < 0)
						lifeindays = 0;
				} else if (strleft(p, "size") &&
					   p[strlen("size")] == '=') {
					keysize = strtol(p + strlen("size") + 1,
							 NULL, 10);
					switch (keysize) {
					case 1024:
					case 2048:
				...
				} else if (strleft(p, "update-stats") &&
					   p[strlen("update-stats")] == '=') {
					buf_clear(statssrc);
					buf_appendf(statssrc, "%s",
						    (p +
						     strlen("update-stats") +
						     1));
					if (statssrc->length > 0) {
						...
						fprintf(stderr,
							"%s: No stats source specified --%s\n",
							argv[0], p);
					}
				} else if ((q = largopt(p, "to", argv[0],
							&error)) != NULL) {
					header = 0;
					buf_appendf(msg, "To: %s\n", q);
				} else if ((q = largopt(p, "post-to", argv[0],
							&error)) != NULL) {
					send = MSG_POST, header = 0;
					buf_appendf(msg, "Newsgroups: %s\n", q);
				} else if ((q = largopt(p, "subject", argv[0],
							&error)) != NULL) {
					buf_appendf(msg, "Subject: %s\n", q);
				} else if ((q = largopt(p, "header", argv[0],
							&error)) != NULL) {
					buf_appendf(msg, "%s\n", q);
				} else if ((q = largopt(p, "chain", argv[0],
							&error)) != NULL) {
					buf_appendf(msg, "Chain: %s\n", q);
				}
#ifdef USE_PGP
				else if ((q = largopt(p, "reply-chain", argv[0],
						      &error)) != NULL) {
					buf_appendf(msg, "Reply-Chain: %s\n",
						    q);
				} else if ((q = largopt(p, "latency", argv[0],
							&error)) != NULL) {
					buf_appendf(msg, "Latency: %s\n", q);
				} else if ((q = largopt(p, "attachment",
							argv[0], &error)) !=
					   NULL) {
					buf_appendf(attachments, "%s\n", q);
#ifdef NYMSUPPORT
				} else if ((q = largopt(p, "nym-config",
							argv[0], &error)) !=
					   NULL) {
					deflt = 0;
					...
					if (i > argc &&
						...
						buf_sets(pseudonym,
						 ...
						 strileft(argv[i + 1], "opt="))
						buf_appends(nymopt,
							    argv[++i] + 5);
				} else if ((q = largopt(p, "nym", argv[0],
							&error)) != NULL) {
					buf_appendf(msg, "Nym: %s\n", q);
#endif /* NYMSUPPORT */
				}
#endif /* USE_PGP */
				else if ((q = largopt(p, "copies", argv[0],
						      &error)) != NULL) {
				...
				while (*++p) {
					...
					case 't':
						...
						if (i < argc - 1)
							...
							fprintf(stderr,
								"%s: Missing argument for option -to\n",
								argv[0]);
							error = 1;
						}
						break;
	...
	if (update_stats) {
		...
		if (ret == -3) {
			...
			fprintf(stderr,
				"Could not open stats source file for writing\n");
		} else if (ret == -1) {
...
\n\
			...
			if (send == MSG_MAIL || send == MSG_POST ||
				...
				while (buf_getheader(msg, field, content) ==
					...
					if (bufieq(field, "nym")) {
						...
						buf_appendheader(sendmsg, field,
								 content);
					}
				}
				buf_nl(sendmsg);
				...
				if (sign || encrypt) {
					...
					if (pgp_mailenc(
						    (encrypt ? PGP_ENCRYPT :
							    ...
							    PGP_TEXT |
							    PGP_REMAIL,
						    sendmsg, nym, pass, NULL,
						    NYMSECRING) != 0) {
						fprintf(stderr,
							"Encryption failed: missing key!");
						ret = 2;
						goto end;
					}
					buf_free(pass);
				...
				if (nym[0] != 0) {
#ifdef NYMSUPPORT
					if (nym_encrypt(sendmsg, nym, send) ==
					    0)
						send = MSG_MAIL;
					else
#endif /* NYMSUPPORT */
						fprintf(stderr,
							"Nym error, sending message anonymously.\n");
				}
#endif /* USE_PGP */
				if (numdest == 0) {
					fprintf(stderr,
	...
	if (send == MSG_NULL) {
		...
		if (mix_encrypt(MSG_NULL, NULL, chain, numcopies, chainlist) ==
		    -1) {
			ret = 2;
			if (chainlist->length)
				printf("%s\n", chainlist->data);
	...
	if (nym[0] != 0) {
		...
		if (nym_config(NYM_CREATE, nym, nymserver, pseudonym, chain,
			       numcopies, chains, nymopt) < 0) {
			ret = 2;
			fprintf(stderr, "Failed!\n");
		}
		user_delpass();
 ...
 * if force != 0 passphrase must match with some key */
{
	...
	if (PASSPHRASE[0] == '\0' && isatty(fileno(stdin)) &&
		...
		while (
#ifdef USE_PGP
			pgpdb_getkey(PK_DECRYPT, PGP_ES_RSA, NULL, NULL, NULL,
				     NULL, NULL, NULL, NULL, NULL, pass) < 0 &&
			pgpdb_getkey(PK_DECRYPT, PGP_E_ELG, NULL, NULL, NULL,
				     NULL, NULL, NULL, NULL, NULL, pass) < 0 &&
#endif /* USE_PGP */
