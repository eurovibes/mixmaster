...
int prepare_type2list(BUFFER *out)
{
	...
	char line[LINELEN], name[LINELEN], addr[LINELEN], keyid[LINELEN],
		version[LINELEN], flags[LINELEN], createdstr[LINELEN],
		expiresstr[LINELEN];
	int assigned;
	...
	while (fgets(line, sizeof(line), list) != NULL) {
		if (strleft(line, begin_key)) {
			...
			assigned = sscanf(
				line,
				"%127s %127s %127s %127s %127s %127s %127s",
				name, addr, keyid, version, flags, createdstr,
				expiresstr);
			if (assigned < 4)
				continue;
			if (assigned >= 6) {
				...
				if (created == 0 || created == -1) {
					errlog(WARNING,
					       "Cannot parse creation date of key %s.\n",
					       keyid);
					continue;
				};
				if (created > time(NULL)) {
					errlog(WARNING,
					       "Key %s created in the future.\n",
					       keyid);
					continue;
				};
			}
			if (assigned >= 7) {
				...
				if (expires == 0 || expires == -1) {
					errlog(WARNING,
					       "Cannot parse expiration date of key %s.\n",
					       keyid);
					continue;
				};
...
int mix2_rlist(REMAILER remailer[], int badchains[MAXREM][MAXREM])
{
	...
	for (n = 1; fgets(line, sizeof(line), list) != NULL && n < MAXREM;)
		if (strlen(line) > 50 && line[0] != '#') {
			...
			for (k = 0; (k < n) && (nw == n); k++) {
				...
				case 1:
					errlog(WARNING,
					       "remailer name %s used with addresses %s and %s\n",
					       name, addr, remailer[k].addr);
					break;
				case 2:
					errlog(WARNING,
					       "remailer address %s used with names %s and %s\n",
				...
				case 3:
                   ...
                   */
					if ((rsalen > remailer[k].rsalen) ||
					    ((rsalen == remailer[k].rsalen) &&
					     (expires) &&
					     (expires < remailer[k].expires) &&
					     (expires > time(NULL)))) {
			...
			fprintf(stderr,
				"STORING nw=%d %s %s has rsalen=%d exp=%d\n",
				nw, remailer[nw].name, textkeyid[nw],
				remailer[nw].rsalen, remailer[nw].expires);
#endif
			if (nw == n)
				n++;
		}
	...
	for (k = 1; k < n; k++) { /* start from 1 */
		if (flag4log[k] && mix_global_verbose)
			fprintf(stderr,
				"For %s keyid %s (%d-bit) was chosen.\n",
				remailer[k].name, textkeyid[k],
				remailer[k].rsalen);
	...
	if (list != NULL) {
		while (fgets(line, sizeof(line), list) != NULL &&
			...
			if (strleft(line, "Last update:")) {
				...
				now = time(NULL);
				if (generated != -1 &&
				    generated < now - SECONDSPERDAY)
					errlog(WARNING,
					       "Remailer Reliability Statistics are older than one day (check your clock?).\n");
				if (generated != -1 && generated > now)
					errlog(WARNING,
					       "Remailer Reliability Statistics are from the future (check your clock?).\n");
			}
		};
		while (fgets(line, sizeof(line), list) != NULL &&
		       !strleft(line, "</PRE>"))
				...
				for (i = 1; i < n; i++)
					if (strleft(line, remailer[i].name) &&
						...
						strncpy(remailer[i]
								...
								.history,
							line + 15, 12);
						remailer[i].info[0].history[12] =
							'\0';
						remailer[i].info[0].reliability =
							10000 * N(line[37]) +
							1000 * N(line[38]) +
							100 * N(line[39]) +
							10 * N(line[41]) +
							N(line[42]);
						remailer[i].info[0].latency =
							36000 * N(line[28]) +
							3600 * N(line[29]) +
							600 * N(line[31]) +
							60 * N(line[32]) +
	...
	parse_badchains(badchains, TYPE2REL, "Broken type-II remailer chains",
			remailer, n);
...
static int send_packet(int numcopies, BUFFER *packet, int chain[], int chainlen,
		       int packetnum, int numpackets, BUFFER *mid,
		       REMAILER remailer[], int badchains[MAXREM][MAXREM],
		       int maxrem, char *redirect_to,
 ...
 */
{
	...
	for (c = 0; c < numcopies; c++) {
		...
		israndom = chain_rand(remailer, badchains, maxrem, thischain,
				      chainlen, 0,
				      ignore_constraints_if_necessary);
		if (israndom == -1) {
			err = -1;
			clienterr(feedback, "No reliable remailers!");
		}
		if ((numcopies > 1 || numpackets > 1) && !israndom &&
		    (chainlen != 1)) {
			clienterr(
				feedback,
		...
		for (hop = 0; hop < chainlen; hop++) {
			...
			case 3: /* not implemented yet; fall back to version 2 */
				...
				if (numcopies > 1 && hop == 0 &&
				    redirect_to == NULL)
					buf_set(encrypted,
						pid); /* same ID only at final hop */
				else
					buf_setrnd(encrypted, 16);
				buf_setrnd(
					key,
					24); /* key for encrypting the body */
				buf_cat(encrypted, key);
				buf_setrnd(iv,
					   8); /* IV for encrypting the body */

				if (hop > 0 || redirect_to != NULL) {
					/* IVs for header chart encryption */
					buf_setrnd(ivarray, 18 * 8);
					buf_cat(ivarray,
						iv); /* 19th IV equals the body IV */

					buf_appendc(
						encrypted,
						0); /* packet type is intermediate */
				...
				if (hop > 0 || redirect_to != NULL) {
					...
					buf_setrnd(
						other,
						19 * 512); /* fill with random data */

				/* timestamp */
				buf_appends(encrypted, "0000");
				buf_appendc(encrypted,
					    '\0'); /* timestamp magic */
				timestamp = time(NULL) / SECONDSPERDAY -
					    rnd_number(4);
				...
				pubkey = buf_new();
				err = db_getpubkey(
					remailer[thischain[hop]].keyid, pubkey);
				if (err == -1)
					goto end;
				...
				if (258 == pubkey->length) {
					...
					err = pk_encrypt(
						key,
						pubkey); /* ... and encrypt the session key */
					whiner++;
				} else {
             ...
             */
					...
					buf_setrnd(
						hkey,
						64); /* compulsory length unless extended by 0s */
               ...
               fprintf(stderr, "        IV=%s\n", showdata(aes_iv,0));
*/
					buf_aescrypt(encrypted, aes_tte_key,
						     aes_iv, ENCRYPT);
					buf_aescrypt(body, aes_body_key, aes_iv,
						     ENCRYPT);
					buf_aescrypt(other, aes_header_key,
						     aes_iv, ENCRYPT);
                   ...
                   */
						...
						buf_reset(antitag);
						buf_append(antitag, other->data,
							   2 * 512);
						buf_reset(temp);
						hmac_sha256(antitag, hkey,
							    temp);
					}
					...
					/* encrypted-328-block */ buf_cat(key,
									  temp);

					/* AES pre key */ buf_cat(key,
								  aes_pre_key);

					err = pk_encrypt(
						key,
						pubkey); /* ... and encrypt the session key etc  */
					whiner += 2;
				}
				...
				if (whiner > 20) {
					clienterr(
						feedback,
						"Chain too long.  Limit is 20 of 1k keys, or 10 of large keys, or somewhere between.");
					err = -1;
					goto end;
				}
				...
				buf_clear(header);
				buf_append(header,
					   remailer[thischain[hop]].keyid, 16);
				...
				buf_pad(header,
					key->length == 128 ?
						512 :
						1024); /* 512 bytes if RSA is 1024 bits */
				if (128 == key->length)
					buf_cat(header, other);
				else
					buf_append(header, other->data,
						   18 * 512);
				break;
			default:
				err = -1;
		...
		if (feedback) {
			for (hop = chainlen - 1; hop >= 0; hop--) {
				buf_appends(feedback,
					    remailer[thischain[hop]].name);
				if (hop > 0)
...
int redirect_message(BUFFER *sendmsg, char *chainstr, int numcopies,
		     BUFFER *feedback)
{
	BUFFER *field;
	BUFFER *content;
	...
	if (chainlen < 1) {
		...
		clienterr(
			feedback,
			"A chainlength of 20 will certainly destroy the message!");
		err = -1;
		goto end;
	};
     ...
     * therefore we need to pick the final remailer with care */
		...
		if (thischain[0] == 0) {
			...
			tempchainlen = chain_select(tempchain, recipient,
						    maxrem, remailer, 0, line);
	 ...
	 * broken chains and DISTANCE */
				...
				israndom = chain_rand(remailer, badchains,
						      maxrem, thischain,
						      chainlen + 1, 0, 0);
				if (israndom == -1) {
					err = -1;
					clienterr(feedback,
						  "No reliable remailers!");
					goto end;
				}
		...
		/* queue the packet */
		if (send_packet(1, sendmsg, thischain, chainlen, -1, -1, NULL,
				remailer, badchains, maxrem, recipient, 0,
				feedback) == -1)
			err = -1;
	};
		 ...
		 int ignore_constraints_if_necessary, BUFFER *feedback)
{
   ...
   ignore_constraints_if_necessary .. to be used when randhopping messages.
	...
	if (chain[0] == 0)
		chain[0] = chain_randfinal(type, remailer, badchains, maxrem, 0,
					   chain, chainlen,
					   ignore_constraints_if_necessary);
	...
	case 2:
		...
		if (type != MSG_NULL) {
			...
			if (temp->length > 10236 &&
			    remailer[chain[0]].flags.compress)
				buf_compress(temp);
			buf_cat(body, temp);
			buf_reset(temp);
		}
		...
		for (i = 0; i <= body->length / 10236; i++) {
			...
			if (send_packet(numcopies, packet, chain, chainlen,
					i + 1, body->length / 10236 + 1, mid,
					remailer, badchains, maxrem, NULL,
					ignore_constraints_if_necessary,
					feedback) == -1)
				err = -1;
		}
