...
void whoami(char *addr, char *defaultname)
{
	...
	(defined(HAVE_GETHOSTNAME) && !defined(HAVE_UNAME))
	char line[LINELEN];
	...
	if (strchr(strchr(addr, '@'), '.') == NULL) {
		...
		if (getdomainname(line, LINELEN) == 0 &&
		    !streq(line, "(none)")) {
			strcatn(addr, ".", LINELEN);
			strcatn(addr, line, LINELEN);
		}
...
/* programs */
	...
	RATE = 100;
	INDUMMYP =
		3; /* add dummy messages with probability p for each message added to the pool */
	OUTDUMMYP =
		10; /* add dummy messages with probability p each time we send from the pool */
	INDUMMYMAXP =
		84; /* for both of the above:  while (rnd < p) { senddummy(); }  */
	OUTDUMMYMAXP =
		96; /* set max INDUMMYP and OUTDUMMYP such that 24 and 5.25 dummy messages will */
	...
	LISTSUPPORTED =
		1; /* list supported remailers in remailer-conf reply? */
	...
	KEYLIFETIME =
		6 * 30 * SECONDSPERDAY; /* default validity period for keys. */
	KEYOVERLAPPERIOD =
		30 * SECONDSPERDAY; /* when keys have this amount of time */
	/* left before expiration, create  */
	/* new ones when ./mix -K is run.*/
	KEYGRACEPERIOD =
		7 * SECONDSPERDAY; /* accept mail to the old key for this */
...
{
	return (read_conf(ADDRESS) || read_conf(NAME) || read_conf(SHORTNAME) ||
		read_conf(REMAILERADDR) || read_conf(ANONADDR) ||
		read_conf(REMAILERNAME) || read_conf(ANONNAME) ||
		read_conf(COMPLAINTS) || read_conf_i(AUTOREPLY) ||
		read_conf(SMTPRELAY) || read_conf(SMTPUSERNAME) ||
		read_conf(SMTPPASSWORD) ||
#ifdef USE_SOCK
		read_conf(HELONAME) || read_conf(ENVFROM) ||
#endif /* USE_SOCK */
		read_conf(SENDMAIL) || read_conf(SENDANONMAIL) ||
		read_conf(PRECEDENCE) || read_conf_i(REMAIL) ||
		read_conf_i(MIX) || read_conf_i(PGP) ||
		read_conf_i(UNENCRYPTED) || read_conf_i(REMIX) ||
		read_conf(NEWS) || read_conf_i(REPGP) || read_conf(EXTFLAGS) ||
		read_conf(MAILtoNEWS) || read_conf(ERRLOG) ||
		read_conf(ORGANIZATION) || read_conf(MID) || read_conf(TYPE1) ||
		read_conf_i(POOLSIZE) || read_conf_i(RATE) ||
		...
		read_conf_i(NUMCOPIES) || read_conf_t(MAILINTIME) ||
		read_conf(CHAIN) || read_conf_i(VERBOSE) ||
		read_conf_i(DISTANCE) || read_conf_i(MINREL) ||
		read_conf_i(RELFINAL) || read_conf_t(MAXLAT) ||
		read_conf_t(MINLAT) || read_conf(PGPPUBRING) ||
		read_conf(PGPSECRING) || read_conf(PASSPHRASE) ||
		read_conf_t(KEYLIFETIME) || read_conf_t(KEYGRACEPERIOD) ||
		read_conf_t(KEYOVERLAPPERIOD) ||
#ifdef USE_SOCK
		...
		read_conf(MSGFOOTERFILE) || read_conf(POP3CONF) ||
		read_conf(HELPFILE) || read_conf(REQUESTDIR) ||
		...
		read_conf(PUBRING) || read_conf(IDLOG) || read_conf(STATS) ||
		read_conf(DESTBLOCK) || read_conf(PGPMAXCOUNT) ||
		read_conf(DESTALLOW) || read_conf(DESTALLOW2) ||
		read_conf(SOURCEBLOCK) || read_conf(STAREX) ||
		read_conf(ALLPINGERSURL) || read_conf(ALLPINGERSFILE) ||
		read_conf(HDRFILTER) || read_conf(REGULAR) || read_conf(POOL) ||
		read_conf(TYPE1LIST) || read_conf(TYPE2REL) ||
		read_conf(PGPREMPUBRING) || read_conf(PGPREMPUBASC) ||
		read_conf(PGPREMSECRING) || read_conf(NYMSECRING) ||
		read_conf(NYMDB) || read_conf(PIDFILE) || read_conf(WGET) ||
		read_conf(STATSSRC) || read_conf_i(STATSAUTOUPDATE) ||
		read_conf_t(STATSINTERVAL) ||

		read_conf_i(CLIENTAUTOFLUSH) || read_conf_i(MAXRECIPIENTS) ||

		read_conf_t(TIMESKEW_FORWARD) || read_conf_t(TIMESKEW_BACK) ||
...
int mix_config(void)
{
  ...
  */
	...
	mixfile(POOLDIR,
		POOL); /* set POOLDIR after reading POOL from cfg file */
	...
	if (RegOpenKeyEx(regsw, "PGP", 0, KEY_ALL_ACCESS, &regpgp) == 0)
		rkey++;
	if (rkey &&
	    RegOpenKeyEx(regpgp, "PGPlib", 0, KEY_QUERY_VALUE, &reg) == 0)
		rkey++;
...
void mix_check_timeskew()
{
	...
	if (f != NULL) {
		...
		if (((TIMESKEW_BACK != 0) && (now < latest - TIMESKEW_BACK)) ||
		    ((TIMESKEW_FORWARD != 0) &&
		     (now > latest + TIMESKEW_FORWARD))) {
			/* Possible timeskew */
			errlog(ERRORMSG,
			       "Possible timeskew detected.  Check clock and rm %s\n",
			       REGULAR);
			exit(TEMP_FAIL);
		}
		fclose(f);
...
int mix_regular(int force)
{
	...
	if (f != NULL) {
		lock(f);
		fscanf(f, "%ld %ld %ld %ld %ld", &tpool, &tpop3, &tdaily,
		       &tmailin, &tstats);
...
int mix_daemon(void)
{
	...
	setsignalhandler(
		1); /* set signal handlers and restart any interrupted system calls */
	for (;;) {
		if (terminatedaemon)
			break;
		if (rereadconfig) {
...
#ifdef WIN32SERVICE
		if (hMustTerminate) {
			if (WaitForSingleObject(hMustTerminate, t * 1000) ==
			    WAIT_OBJECT_0) {
		...
		if (!terminatedaemon && !rereadconfig) {
			setsignalhandler(
				0); /* set signal handlers;  don't restart system calls */
#ifdef WIN32
...
void errlog(int type, char *fmt, ...)
{
	...
	char line[LINELEN];
	int p;
	char err[6][8] = { "", "Error", "Warning", "Notice", "Info", "Info" };

	if ((VERBOSE == 0 && type != ERRORMSG) ||
	    (type == LOG && VERBOSE < 2) || (type == DEBUGINFO && VERBOSE < 3))
		return;
	...
	if (e == NULL &&
	    (ERRLOG[0] == '\0' || (e = mix_openfile(ERRLOG, "a")) == NULL))
