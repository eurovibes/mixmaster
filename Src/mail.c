...
int sendmail(BUFFER *message, char *from, BUFFER *address)
{
	...
	if (!rcpt_cnt) {
		...
		int i;
		for (i = 0; i < 10000; i++) {
			snprintf(path, PATHMAX, "%s%cout%i.txt", POOLDIR,
				 DIRSEP, i);
			f = fopen(path, "r");
			if (f)
				fclose(f);
		...
		for (count = 0;; count++) {
			snprintf(path, PATHMAX,
				 "%s%cout.%lu.%u_%lu.%s,S=%lu.txt", POOLDIR,
				 DIRSEP, time(NULL), getpid(), namecounter++,
				 hostname, head->length + message->length);

			if (stat(path, &statbuf) == 0)
				errno = EEXIST;
			else if (errno ==
				 ENOENT) { /* create the file (at least try) */
				f = fopen(path, "w");
				if (f != NULL)
...
SOCKET smtp_open(void)
{
	...
	if (s != INVALID_SOCKET) {
		...
		if (line->data[0] != '2') {
			...
			if (line->data[0] != '2') {
				errlog(ERRORMSG,
				       "SMTP relay refuses HELO: %b\n", line);
				closesocket(s);
				s = INVALID_SOCKET;
			} else if (SMTPUSERNAME[0] && esmtp &&
				   bufifind(line, "AUTH") &&
				   bufifind(line, "LOGIN")) {
...
int smtp_send(SOCKET relay, BUFFER *head, BUFFER *message, char *from)
{
	...
	while (buf_isheader(message) &&
	       buf_getheader(message, field, content) == 0) {
		if (bufieq(field, "to") || bufieq(field, "cc") ||
		    bufieq(field, "bcc")) {
#ifdef BROKEN_MTA
			if (!bufifind(rcpt, content->data))
...
SOCKET pop3_open(char *user, char *host, char *pass, int auth)
{
	...
	if (server != INVALID_SOCKET) {
		...
		if (!authenticated) {
			...
			if (!bufleft(line, "+"))
				errlog(ERRORMSG,
				       "POP3 USER command at %s failed: %b\n",
				       host, line);
			else {
...
void pop3get(void)
{
	...
	if (f != NULL)
		while (fgets(cfg, sizeof(cfg), f) != NULL) {
			...
			if (strchr(cfg, '@'))
				strchr(cfg, '@')[0] = ' ';
			if (sscanf(cfg, "%127s %127s %127s %4s", user, host,
				   pass, auth) < 3)
				continue;
			i = POP3_ANY;
					...
					for (i = 1; i <= num; i++) {
						if (POP3SIZELIMIT > 0 &&
							...
							if (pop3_retr(server, i,
								...
								errlog(WARNING,
								       "POP3 error while getting mail from %s.",
								       host);
								closesocket(
									server);
								goto end;
