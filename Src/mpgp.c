...
int decrypt(BUFFER *u, BUFFER *option, char *n)
{
	...
	case PGP_SIGNKEY:
		fprintf(stderr,
			"%s: Unknown signature key %s, cannot verify.\n", n,
			sig->data);
		err = 1;
		break;
	case PGP_SIGBAD:
...
int main(int argc, char *argv[])
{
	...
	if (argc == 1 ||
	    (cmd > 0 && (cmd[1] == 'e' || cmd[1] == 'c' || cmd[1] == 'd' ||
			 cmd[1] == 'a' || cmd[1] == 's' || cmd[1] == 'C'))) {
	...
	if (cmd)
		switch (cmd[1]) {
		case 's':
			err = pgp_encrypt(PGP_SIGN | (text ? PGP_TEXT : 0), u,
					  NULL, option, NULL, PGPPUBRING,
					  PGPSECRING);
			if (err != 0) {
				pass(pp);
				err = pgp_encrypt(PGP_SIGN |
							  (text ? PGP_TEXT : 0),
						  u, NULL, option, pp,
		...
		case 'e':
			if (option->length) {
				err = pgp_encrypt(PGP_ENCRYPT |
							  (text ? PGP_TEXT : 0),
						  u, option, NULL, NULL,
						  PGPPUBRING, PGPSECRING);
				if (err < 0)
					fprintf(stderr,
						"%s: can't encrypt message for %s\n",
						argv[0], argv[2]);
			}
			break;
		case 'c':
			pass(option);
			err = pgp_encrypt(
				PGP_CONVENTIONAL | (text ? PGP_TEXT : 0), u,
				option, NULL, NULL, PGPPUBRING, PGPSECRING);
			if (err < 0)
		...
		case 'C':
			pass(option);
			err = pgp_encrypt(
				PGP_NCONVENTIONAL | (text ? PGP_TEXT : 0), u,
				option, NULL, NULL, PGPPUBRING, PGPSECRING);
			if (err < 0)
				fprintf(stderr, "%s: can't encrypt message\n",
					argv[0]);
		...
		case 'V':
			...
			fprintf(stderr,
				"(C) 2000 - 2004 Ulf Moeller and others.\n");
			fprintf(stderr,
