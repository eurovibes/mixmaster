	...
	if (d != NULL)
		for (;;) {
			...
			if (e == NULL)
				break;
			if (is(e->d_name, "lat")) {
				sprintf(path, "%s%c%s", POOLDIR, DIRSEP,
					e->d_name);
				f = fopen(path, "rb");
				if (f != NULL) {
					fscanf(f, "%*d %ld\n", &then);
...
#ifdef SHORTNAMES
	sprintf(tmpname, "%s%c%02x%02x%02x%02x.tmp", POOLDIR, DIRSEP,
		rnd_byte(), rnd_byte(), rnd_byte(), rnd_byte());
	strcpy(path, tmpname);
	memcpy(path + strlen(path) - 3, type, 3);
#else /* end of SHORTNAMES */
	sprintf(tmpname, "%s%ct%02x%02x%02x%02x%02x%02x%01x", POOLDIR, DIRSEP,
		rnd_byte(), rnd_byte(), rnd_byte(), rnd_byte(), rnd_byte(),
...
int create_dummy_mailout()
{
	while (rnd_number(100) < OUTDUMMYP) {
		errlog(DEBUGINFO,
		       "Generating dummy message with outgoing mail.\n");
	...
	if (type == INTERMEDIATE) {
		...
		if (MIDDLEMAN && !allowmessage(m)) {
			...
			if (err != -1) {
				...
				errlog(DEBUGINFO,
				       "Sending message (%ld bytes)\n",
				       m->length);

				if (type == MSG_MAIL) {
					err = sendmail(
						m, userfrom ? NULL : ANONNAME,
						NULL);
					stats_log(4);
...
int filtermsg(BUFFER *in)
{
	...
	while (buf_getline(in, line) == 0) {
		...
		if (bufileft(line, "content-type:") &&
		    bufileft(line, "multipart"))
	...
	if (MID[0] != '\0' && tolower(MID[0]) != 'n') {
		...
		if (MID[0] == '@')
			...
			if (strchr(REMAILERADDR, '@'))
				strcatn(txt, strchr(REMAILERADDR, '@'),
					sizeof(txt));
			else if (strchr(COMPLAINTS, '@'))
				strcatn(txt, strchr(COMPLAINTS, '@'),
					sizeof(txt));
		}
  ...
  buf_append(out, in->data + in->ptr, in->length - in->ptr);
	...
	while (buf_getline(in, line) != -1) {
		if (boundary(line, mboundary)) {
			buf_cat(out, line);
			buf_nl(out);
			while (buf_getline(in, line) ==
			       0) { /* MIME body part header */
