...
int url_download(char *url, char *dest)
{
...
#else
	{
		...
		if (0 != strncmp(url, "http", 4)) {
			...
			errlog(ERRORMSG,
			       "refusing external program %s with url=%s dest=%s\n",
			       WGET, url, dest);
			return 1;
		}
 ...
 */
int get_attribute(BUFFER *inifile, char *section, char *attribute,
		  BUFFER *value)
{
	BUFFER *line;
	int err = -1;
...
int stats_download(BUFFER *allpingers, char *sourcename, int curses)
{
	...
	for (i = 0; i < NUMFILES; i++) {
		...
		if (curses) {
			mvprintw(i + 3, 0, "downloading %s from %s...",
				 localfiles[i], value->data);
			refresh();
		} else
...
void update_stats()
{
	...
	while (1) {
		...
		if (statssrc->length > 0)
			printw("       Current: %s (Enter to download)",
			       statssrc->data);
		printw("\n\n");
		...
		mvprintw(
			y, 0,
			"*  update list of pingers from web     =  edit list       <space> to exit");
		c = getch();
		if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) {
			if (c >= 'a')
				c -= 'a';
