...
int mix_decrypt(BUFFER *message)
{
	...
	for (;;) {
		...
		if (bufieq(field, "to"))
			...
			if (bufieq(content, "help") ||
			    bufieq(content, "remailer-help"))
				type = REQUESTHELP;
		...
		} else if (bufieq(field, "test-to") ||
			   bufieq(field, "encrypted") ||
			   bufieq(field, "anon-to") ||
			   bufieq(field, "request-remailing-to") ||
			   bufieq(field, "remail-to") ||
			   bufieq(field, "anon-post-to") ||
			   bufieq(field, "post-to") ||
			   bufieq(field, "anon-send-to") ||
			   bufieq(field, "send-to") ||
			   bufieq(field, "remix-to") ||
...
hdrend:
	...
	default:
		if (strifind(replyto->string, "mailer-daemon")) {
			...
			if (AUTOREPLY)
				sendinfofile(USAGEFILE, USAGELOG, replyto,
					     NULL);
...
int create_dummy_mailin()
{
	while (rnd_number(100) < INDUMMYP) {
		errlog(DEBUGINFO,
		       "Generating dummy message with incoming mail.\n");
...
int t2_decrypt(BUFFER *in)
{
	...
	do {
		...
		if (err != -1) {
		 ...
		 in->length); /* accept several packets in one message */
...
/* create a filename */
{
	...
	sprintf(fname, "%s%cp%02x%02x%02x%02x%02x%02x%01x", POOLDIR, DIRSEP,
		packetnum, mid->data[0], mid->data[1], mid->data[2],
		mid->data[3], mid->data[4], mid->data[5] & 15);
...
void pool_packetexp(void)
{
	...
	if (d != NULL)
		for (;;) {
			...
			if (e == NULL)
				break;
			if (e->d_name[0] == 'p' || e->d_name[0] == 'e' ||
			    e->d_name[0] == 't') {
				path = malloc(strlen(POOLDIR) +
					      strlen(e->d_name) +
					      strlen(DIRSEPSTR) + 1);
				if (path) {
					strcpy(path, POOLDIR);
					...
					if (stat(path, &sb) == 0 &&
						...
						if (e->d_name[0] == 'p') {
							errlog(NOTICE,
							       "Expiring incomplete partial message %s.\n",
							       e->d_name);
						} else if (e->d_name[0] ==
							   'e') {
							errlog(NOTICE,
							       "Expiring old error message %s.\n",
							       e->d_name);
						} else if (e->d_name[0] ==
							   't') {
							errlog(NOTICE,
							       "Expiring moldy temporary message %s.\n",
							       e->d_name);
						}
						unlink(path);
					}
...
void logmail(char *mailbox, BUFFER *message)
{
	...
	if (strieq(mailbox, "stdout"))
		...
		while (buf_getheader(message, field, content) == 0)
			if (bufieq(field, "x-loop") &&
			    bufifind(content, REMAILERADDR)) {
				errlog(WARNING,
				       "Loop detected! Message not sent to %s.\n",
				       mailbox);
				goto isloop;
			}
	...
	isloop:
		...
		if (!bufileft(message, "From ")) {
			t = time(NULL);
			tc = localtime(&t);
			strftime(line, LINELEN,
				 "From Mixmaster %a %b %d %H:%M:%S %Y\n", tc);
			fprintf(mbox, "%s", line);
		}
...
{
	int request = 0, num, i;
	BUFFER *from, *line, *field, *content, *addr, *remailer_addr,
		*copy_addr;
	...
	while (buf_getline(message, line) != -1)
		if (bufifind(line, "destination-block")) {
			...
			if (doblock(remailer_addr, copy_addr, 1)) {
				errlog(LOG,
				       "Ignoring blocking request for %b from %b.\n",
				       addr, from);
				request = 2;
				goto end;
			}
			...
			if (AUTOBLOCK) {
				...
				if (line->length == 0) {
					errlog(LOG,
					       "Nothing to block after rfc822_addr().\n");
					...
					if (strchr(line->string, '@') &&
						...
						errlog(WARNING,
						       "Invalid address not added to %s: %b\n",
						       DESTBLOCK, addr);
				}
			}
		}
...
int idexp(void)
{
	...
	i = lockfile(IDLOG);
	now = time(NULL);
	if (fread(&idbuf, 1, sizeof(idlog_t), f) !=
	    sizeof(idlog_t)) { /* replace first line */
		fclose(f);
		unlockfile(i);
		return (-1);
	}
	...
	fseek(f, fpi,
	      SEEK_SET); /* this fseek does nothing, but MSVC CRT happilly reads past EOF (!!!) if we do not fseek here :-/ */
	while (fread(&idbuf, 1, sizeof(idlog_t), f) == sizeof(idlog_t)) {
		fpi += sizeof(idlog_t);
		then = idbuf.time;
		if (now - then < IDEXP && now - then > -SECONDSPERDAY * 180)
...
int pgpmaxexp(void)
{
	...
	while (fgets(temp, sizeof(temp), f) != NULL)
		if (sscanf(temp, "%ld", &then) && then >= now - SECONDSPERDAY)
			buf_appends(b, temp);
