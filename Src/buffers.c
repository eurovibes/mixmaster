...
{
	assert(from != to);
	return (buf_append(to, from->data + from->ptr,
			   from->length - from->ptr));
}
...
int buf_read(BUFFER *outmsg, FILE *infile)
{
	...
	for (;;) {
		...
		if (outmsg->length > BUFFER_MAX) {
			errlog(ERRORMSG,
			       "Message file too large. Giving up.\n");
			return (1);
		}
#endif /* BUFFER_MAX */
	}
...
int buf_getline(BUFFER *buffer, BUFFER *line)
{
	...
	for (i = buffer->ptr; i < buffer->length; i++) {
		...
		if (buffer->data[i] == '\r' && i + 1 <= buffer->length &&
		    buffer->data[i + 1] == '\n') {
			nl = 2;
			break;
		}
...
/* equivalent to buf_set(dest, src); buf_reset(src); */
{
	...
	dest->data = src->data, dest->size = src->size,
