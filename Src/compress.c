...
static byte gz_magic[2] = { 0x1f, 0x8b }; /* gzip magic header */
...
int buf_unzip(BUFFER *in, int type)
{
	...
	s.avail_in =
		in->length + 1 - in->ptr; /* terminating 0 byte as "dummy" */
	s.next_out = NULL;
	...
	if (ret != 0)
		...
		case Z_STREAM_ERROR:
			errlog(ERRORMSG,
			       "Decompression error Z_STREAM_ERROR.\n", err);
			break;
		case Z_MEM_ERROR:
		...
		case Z_VERSION_ERROR:
			errlog(ERRORMSG,
			       "Decompression error Z_VERSION_ERROR.\n", err);
			break;
		default:
...
int buf_zip(BUFFER *out, BUFFER *in, int bits)
{
	...
	if (deflateInit2(&s, Z_DEFAULT_COMPRESSION, Z_DEFLATED, -bits, 8, 0) !=
	    Z_OK)
		goto end;
	...
	buf_append(out, NULL,
		   (int)13 + in->length * 1.01); /* fit it in one chunk */
