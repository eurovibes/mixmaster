...
redraw:
	...
	for (;;) {
		...
		printw(type == 'p' || type == 'f' ? "posting to Usenet" :
						    "sending mail");
		standend();
		mix_status(NULL);
		cl(2, 0);
		...
		if (!strleft(thisnym, NONANON)) {
			chain_reliability(chain, 0,
					  reliability); /* chaintype 0=mix */
			cl(4, 0);
			printw("c)hain: %-35s (reliability: %s)", chain,
			       reliability);
			cl(5, 0);
		...
		if (c != ERR) {
			...
			edit:
				linecount = 1;
				sprintf(path, "%s%cx%02x%02x%02x%02x.txt",
					POOLDIR, DIRSEP, rnd_byte(), rnd_byte(),
					rnd_byte(), rnd_byte());
				f = fopen(path, "w");
				if (f == NULL) {
#ifdef USE_NCURSES
