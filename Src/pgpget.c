	       ...
	       char *secring)
{
	...
	while ((type = pgp_getpacket(in, p)) > 0)
		...
		case PGP_ENCRYPTEDMDC:
			...
			if (key->length > 0)
				err = pgp_getsymmetric(
					p, key, algo, type == PGP_ENCRYPTEDMDC);
			else
				err = -1;
			if (err == 0)
				err = pgp_getmsg(p, NULL, sig, pubring,
						 secring);
			if (err != PGP_ERR)
				buf_move(out, p);
			break;
	...
	if (sig) {
		if (signature.ok == PGP_SIGNKEY)
			buf_appendf(sig, "%02X%02X%02X%02X",
				    signature.userid->data[4],
				    signature.userid->data[5],
				    signature.userid->data[6],
...
int pgp_getsig(BUFFER *p, pgpsig *sig, char *pubring)
{
	...
	if (hashalgo != PGP_H_MD5)
		goto end;
	hash = buf_geti(p);
	if (pgpdb_getkey(PK_VERIFY, algo, NULL, NULL, NULL, sigkey, NULL,
			 sig->userid, id, pubring, NULL) < 0) {
		sig->ok = PGP_SIGNKEY;
		if (sig->userid)
...
static int pgp_ideadecrypt(BUFFER *in, BUFFER *out, BUFFER *key, int mdc)
{
	...
	idea_cfb64_encrypt(in->data + 10 + mdc, out->data,
			   in->length - 10 - mdc, &ks, iv, &n, IDEA_DECRYPT);
	if (mdc) {
		if (out->length > 22) {
			out->length -= 22;
			if (out->data[out->length] == 0xD3 &&
			    out->data[out->length + 1] == 0x14) {
...
static int pgp_castdecrypt(BUFFER *in, BUFFER *out, BUFFER *key, int mdc)
{
	...
	CAST_cfb64_encrypt(in->data + 10 + mdc, out->data,
			   in->length - 10 - mdc, &ks, iv, &n, CAST_DECRYPT);
	if (mdc) {
		if (out->length > 22) {
			out->length -= 22;
			if (out->data[out->length] == 0xD3 &&
...
static int pgp_bfdecrypt(BUFFER *in, BUFFER *out, BUFFER *key, int mdc)
{
	...
	BF_cfb64_encrypt(in->data + 10 + mdc, out->data, in->length - 10 - mdc,
			 &ks, iv, &n, BF_DECRYPT);
	if (mdc) {
		if (out->length > 22) {
			out->length -= 22;
...
static int pgp_aesdecrypt(BUFFER *in, BUFFER *out, BUFFER *key, int mdc)
{
	...
	AES_cfb128_encrypt(in->data + 18 + mdc, out->data,
			   in->length - 18 - mdc, &ks, iv, &n, AES_DECRYPT);
	if (mdc) {
		if (out->length > 22) {
			out->length -= 22;
			if (out->data[out->length] == 0xD3 &&
...
int pgp_getsessionkey(BUFFER *in, BUFFER *pass, char *secring)
{
	...
	buf_get(in, keyid, 8);
	algo = buf_getc(in);
	err = pgpdb_getkey(PK_DECRYPT, algo, NULL, NULL, &expires, key, NULL,
			   NULL, keyid, secring, pass);
	if (err < 0)
		goto end;
