...
int pgpdb_close(KEYRING *keydb)
{
	...
	if (keydb->modified) {
		...
		assert(keydb->type == PGP_TYPE_PRIVATE ||
		       keydb->type == PGP_TYPE_PUBLIC);
		if (keydb->filetype == ARMORED)
			pgp_armor(keydb->db, keydb->type == PGP_TYPE_PUBLIC ?
						     PGP_ARMOR_KEY :
						     PGP_ARMOR_SECKEY);
		if (keydb->filetype == -1 ||
		    (f = mix_openfile(
			     keydb->filename,
			     keydb->filetype == ARMORED ? "w" : "wb")) == NULL)
			err = -1;
		else {
...
/* store next key from keydb with specified keyid/userid in key. */
{
	...
	while (!found) {
		...
		while ((ptr = keydb->db->ptr,
			type = pgp_getpacket(keydb->db, p)) > 0) {
			switch (type) {
			case PGP_SECKEY:
			case PGP_PUBKEY:
				keydb->db->ptr = ptr;
...
int pgpdb_getkey(int mode, int algo, int *sym, int *mdc, long *expires,
		 BUFFER *key, BUFFER *userid, BUFFER *founduid, BUFFER *keyid,
 ...
 *        a key is not unique */
{
	...
	for (;;) {
		...
		thisalgo = pgp_getkey(mode, algo, sym, mdc, expires, thiskey,
				      thiskey, thisid, founduid, pass);
		if (thisalgo == PGP_PASS)
			needpass = 1;
		if (thisalgo > 0) {
			found++;
			if ((thisalgo == pgp_preferredalgo &&
			     algofound != pgp_preferredalgo && algofound > 0) ||
			    (thisalgo != pgp_preferredalgo &&
			     algofound == pgp_preferredalgo))
	...
	if (found < 1) {
		if (needpass)
			 ...
			 *sym != PGP_K_IDEA) { /* kludge: try again with 3DES */
			if (userid)
				errlog(NOTICE, "Key %b not found!\n", userid);
			else if (keyid && keyid->length > 7)
				errlog(NOTICE,
				       "Key %02X%02X%02X%02X not found!\n",
				       keyid->data[4], keyid->data[5],
				       keyid->data[6], keyid->data[7]);
		}
	}
	if (found > 1) {
		if (userid)
			...
			errlog(ERRORMSG, "Key %02X%02X%02X%02X not unique!\n",
			       keyid->data[4], keyid->data[5], keyid->data[6],
			       keyid->data[7]);
		else
...
int pgp_keymgt(int force)
{
   ...
   */
	res = pgpdb_getkey(PK_DECRYPT, PGP_ES_RSA, NULL, NULL, &expires, NULL,
			   NULL, NULL, NULL, NULL, pass);
	if (force == 2 || res < 0 ||
	    (expires > 0 && expires - KEYOVERLAPPERIOD < time(NULL))) {
		rsa_ok = -1;
		pgp_keygen(PGP_ES_RSA, 0, userid, pass, PGPKEY, PGPREMSECRING,
			   0);
	};

	if (force == 0 &&
	    (pgpdb_getkey(PK_ENCRYPT, PGP_ES_RSA, NULL, NULL, NULL, NULL, NULL,
   ...
   *        is before another matching key in the keyring tho
   */
	res = pgpdb_getkey(PK_DECRYPT, PGP_E_ELG, NULL, NULL, &expires, NULL,
			   NULL, NULL, NULL, NULL, pass);
	if (force == 2 || res < 0 ||
	    (expires > 0 && expires - KEYOVERLAPPERIOD < time(NULL))) {
		dsa_ok = -1;
		pgp_keygen(PGP_E_ELG, 0, userid, pass, PGPKEY, PGPREMSECRING,
			   0);
	}

	if (force == 0 &&
	    (pgpdb_getkey(PK_ENCRYPT, PGP_E_ELG, NULL, NULL, NULL, NULL, NULL,
			  NULL, NULL, PGPKEY, NULL) > 0) &&
	    dsa_ok == 0)
		dsa_ok = 1;
   ...
   * versions happy */
	...
	if (keys == NULL)
		...
		while (pgpdb_getnext(keys, key, NULL, userid) != -1) {
			buf_clear(outtxt);
			if (pgp_makekeyheader(PGP_PUBKEY, key, outtxt, NULL,
					      PGP_ANY) == 0) {
				err = 0;
				buf_appends(
					out,
	...
	while (pgpdb_getnext(keys, key, NULL, userid) != -1) {
		...
		buf_clear(keybak);
		buf_cat(keybak, key);
		if (pgp_makekeyheader(PGP_SECKEY, key, outtxt, pass, PGP_ANY) ==
		    0) {
			err = 0;
			buf_appends(secout,
				    "Type Bits/KeyID     Date       User ID\n");
		...
		buf_clear(outtxt);
		if (recreate_pubring && pgp_makepubkey(keybak, outtxt, outkey,
						       pass, PGP_ANY) == 0) {
			buf_appends(out,
				    "Type Bits/KeyID     Date       User ID\n");
			buf_cat(out, outtxt);
 ...
 */
{
	...
	if (keys != NULL) {
		while (pgpdb_getnext(keys, key, NULL, userid) != -1) {
			buf_clear(tmptxt);
			if (pgp_makekeyheader(PGP_PUBKEY, key, tmptxt, NULL,
					      algo) == 0) {
				buf_rewind(key);
				pgp_getkey(PK_VERIFY, algo, NULL, NULL,
					   &expires, key, NULL, NULL, NULL,
					   NULL);
				if (expires == 0 ||
