	       ...
	       BUFFER *options)
{
	...
	BUFFER *userid, *msg, *req, *k, *line, *ek, *eklist, *key, *pubkey,
		*out, *oldchains;
	int latency;
	int err = -1;
	int status;
	...
	if (nymserver) {
		...
		if (maxrem < 1)
			return (-1);
		if (chain_select(chain, nymserver, maxrem, remailer, 2, NULL) !=
		    1)
			return (-1);
		if (chain[0] == 0)
			chain[0] = chain_randfinal(MSG_MAIL, remailer,
						   badchains, maxrem, 2, NULL,
						   -1);
		if (chain[0] == -1)
			return (-1);
		assert(strchr(nym, '@') == NULL &&
		       strchr(remailer[chain[0]].addr, '@'));
		strcatn(nym, strchr(remailer[chain[0]].addr, '@'), LINELEN);
		buf_appends(config, remailer[chain[0]].addr);
	} else
		assert(strchr(nym, '@') != NULL);

	status = nymlist_getnym(nym, config->length ? NULL : config, eklist,
				NULL, NULL, oldchains);
	...
	getkey:
		...
		if (err != 0) {
			...
			pgp_keygen(PGP_ES_RSA, 0, userid, userpass, NULL,
				   NYMSECRING, 2);
			goto getkey;
		}
	...
	if (mode != NYM_DELETE) {
		...
		if (chains)
			for (;;) {
				...
				if (err != 0 && rblock == 1) {
					buf_setrnd(ek, 16);
					if (t1_encrypt(desttype, msg, rchain,
						       latency, ek,
						       NULL) != 0) {
						err = -2;
						goto end;
	...
	err = pgp_encrypt(PGP_ENCRYPT | PGP_SIGN | PGP_TEXT | PGP_REMAIL, req,
			  line, userid, userpass, NULL, NYMSECRING);
	if (err != 0)
		goto end;
#ifdef DEBUG
	buf_write(req, stderr);
...
#endif /* DEBUG */
		user_pass(userpass);
		err = pgp_encrypt(PGP_ENCRYPT | PGP_SIGN | PGP_TEXT |
					  PGP_REMAIL,
	...
	while ((ptr = msg->ptr, err = buf_getline(msg, line)) != -1) {
		if (bufleft(line, begin_pgpmsg)) {
			...
			if (pgp_isconventional(pgpmsg)) {
				...
				if (nymlist_read(nymlist) == -1)
					user_delpass();
				while (nymlist_get(nymlist, nym, config, eklist,
						   opt, name, rblocks,
						   &status) >= 0) {
					while (buf_getline(eklist, ek) == 0) {
						decode(ek, ek);
						if (t1_getreply(pgpmsg, ek,
								20) == 0) {
							buf_clear(out);
							err = pgp_decrypt(
								pgpmsg,
			...
			if (decrypted == 0) {
				user_pass(userpass);
				err = pgp_decrypt(pgpmsg, userpass, sig,
...
int nymlist_write(BUFFER *list)
{
	...
	user_pass(key);
	if (key->length)
		pgp_encrypt(PGP_NCONVENTIONAL | PGP_NOARMOR, list, key, NULL,
			    NULL, NULL, NULL);
	buf_free(key);
#endif /* USE_PGP */
...
int nymlist_del(BUFFER *list, char *nym)
{
	...
	while (nymlist_get(list, thisnym, config, ek, opt, name, rblocks,
		...
		if (!streq(nym, thisnym))
			nymlist_append(new, thisnym, config, opt, name, rblocks,
				       ek, thisstatus);
