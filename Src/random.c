...
int rnd_bytes(byte *b, int n)
{
	...
	if (n + idx < BUFSIZE) {
		...
		if (1 != RAND_bytes(b, n))
			errlog(WARNING,
			       "deficient randomness while filling buffer\n");
	}

	if (idx + 256 > BUFSIZE) {
		if (1 != RAND_bytes(rand, BUFSIZE))
			errlog(WARNING,
			       "deficient randomness while filling static cache\n");
		idx = 0;
	}
	return (0);
}
...
int rnd_number(int n)
{
	...
	if (n > 65535)
		do
			r = rnd_byte() * 65536 + rnd_byte() * 256 + rnd_byte();
		while (r >= n);
	else if (n > 255)
		do
