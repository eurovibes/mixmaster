 ...
 */
{
	...
	if (now - old < 5 * SECONDSPERDAY) /* never reject messages less than */
		old = now -
		      5 * SECONDSPERDAY; /* 5 days old (== minimum IDEXP) */
	...
	if (ftell(f) % sizeof(idlog_t))
		fseek(f, 0 - (ftell(f) % sizeof(idlog_t)),
		      SEEK_CUR); /* make sure that we're on sizeof(idlog_t) byte boundary */
	memcpy(idbuf.id, id->data, sizeof(idbuf.id));
	idbuf.time = now;
	fwrite(&idbuf, 1, sizeof(idlog_t), f);
		...
		if (old_day == now_day) {
			...
			if ((tf = mix_openfile(RSATEXTFILE, "a")) != NULL) {
				...
				fprintf(tf, "%04d-%02d-%02d %6d %6d %6d %6d\n",
					1900 + gt->tm_year, 1 + gt->tm_mon,
					gt->tm_mday, rs.r[1], rs.r[2], rs.r[3],
					rs.r[4]);
				fclose(tf);
				ri = 0, rj = 0, flag = 0;
				if ((tf = mix_openfile(RSATEXTFILE, "r")) !=
				    NULL) {
				...
				if (flag) {
					errlog(NOTICE,
					       "rotating file %s from line %d\n",
					       RSATEXTFILE, ri);
					if ((tf = mix_openfile(RSATEXTFILE,
							       "w")) != NULL) {
						do {
							fprintf(tf, "%s",
								queue[ri]);
      ...
      * -2: old message */
{
	...
	BUFFER *ttedigest, *hkey, *aes_pre_key, *aes_header_key, *aes_body_key,
		*aes_tte_key, *aes_iv;
	BUFFER *trail;
	...
	assert(128 == rsalen || 256 == rsalen || 384 == rsalen ||
	       512 == rsalen);
	...
	if (128 == rsalen) {
		if (extract->length != 24) {
			err = -1;
			errlog(NOTICE,
			       "Cannot decrypt message - RSA 1024 data has wrong length %d not 24.\n",
			       extract->length);
		...
		if (extract->length != 216) {
			err = -1;
			errlog(NOTICE,
			       "Cannot decrypt message - RSA (large key) data has wrong length %d.\n",
			       extract->length);
			/* supposed to be:
         * 3DES
         * hmac key
		...
		if (!buf_eq(bodydigest, temp)) {
			errlog(NOTICE,
			       "Antitagging test - wrong digest on body.\n");
			err = -1;
			goto end;
		}
		buf_reset(temp);
		...
		if (!buf_eq(ttedigest, temp)) {
			errlog(NOTICE,
			       "Antitagging test - wrong digest on 328-block.\n");
			err = -1;
			goto end;
		}
		/* There is one more test applicable if packet type is 0. */

		derive_aes_keys(aes_pre_key, hkey, aes_header_key, aes_body_key,
				aes_tte_key, aes_iv);
	...
	case 0:
		if (rsalen >= 256) {
			...
			if (!buf_eq(otherdigest, temp)) {
				errlog(NOTICE,
				       "Antitagging test - wrong digest on later header\n");
				err = -1;
				goto end;
			}
		}
	...
	buf_get(dec, digest,
		16); /* digest of this block, but not so far as to include the digest  */
	...
	case 0:
		err = -2; /* redundant message */
		goto end;
	case -1:
		err = -1; /* future timestamp */
		goto end;
	}
	...
	case 0:
		...
		buf_reset(header);
		buf_pad(header,
			512); /* one block of 512 random data regardless of RSA key size */
		buf_cat(out, header);
		buf_reset(iv);
	...
	for (i = 0; i < n; i++) {
		...
		if (bufileft(line, "post:")) {
			...
			if (line->length > 5) {
				...
				buf_append(newsgroups, line->data + j,
					   line->length - j);
			}
		} else {
			if (to->length > 0)
