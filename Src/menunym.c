...
void menu_nym(char *nnym)
{
	...
	if (nymlist_read(nymlist) == -1) {
		...
		pass = 1;
	while (nymlist_get(nymlist, nym[num], NULL, NULL, NULL, NULL, NULL,
			   &s) >= 0) {
		if (s == NYM_OK) {
			if (num < maxnym)
				num++;
...
select:
	...
	if (c >= '0' && c <= '9') {
		...
		if (edit ? select == 0 || select >= num + numpending - 1 :
			   select >= num) {
			beep();
			select = -1;
		}
		...
		case 'c':
			if (pass) {
				...
				int i;
				char line[LINELEN];
				int acksend = 0, signsend = 0, fixedsize = 0,
				    disable = 0, fingerkey = 1;
				...
				if (edit)
					strncpy(alias,
						...
						LINELEN);
				if (edit == NYM_MODIFY) {
					nymlist_getnym(alias, NULL, NULL, opt,
						       name, NULL);
			...
			loop : {
				...
				chain_reliability(
					sendchain, 0,
					reliability); /* chaintype 0=mix */
				printw("     c)hain to nym server: %-30s (reliability: %s)",
				       sendchain, reliability);
				cl(8, 0);
				printw("     n)umber of redundant copies: %d",
				       sendnumcopies);
				if (edit != NYM_DELETE) {
					...
					if (rnum == -1)
						...
						printw("     number of r)eply chains: %2d                                     reliability",
						       rnum);
						for (i = 0; i < rnum; i++) {
							cl(i + 19, 0);
							chain_reliability(
				...
				create:
					...
					if (edit) {
						mix_status(
							"Preparing nymserver configuration message...");
						err = nym_config(edit, alias,
								 NULL, name,
								 sendchain,
								 sendnumcopies,
								 chains, opt);
						...
						mix_status(
							"Preparing nym creation request...");
						err = nym_config(edit, alias,
								 nymserv, name,
								 sendchain,
								 sendnumcopies,
					...
					if (err != 0) {
						...
						if (edit)
							mix_status(
								"Nymserver configuration message completed.");
						else
							mix_status(
								"Nym creation request completed.");
					}
					if (chains)
						buf_free(chains);
					goto end;
