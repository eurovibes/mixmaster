...
int main(int argc, char *argv[])
{
	...
	if (!StartServiceCtrlDispatcher(dispatchTable)) {
		...
		event_log(
			1000,
			"Service not started: StartServiceCtrlDispatcher failed");
	}
	return 0;
} /* main */
...
VOID WINAPI service_main(DWORD argc, LPSTR *argv)
{
	...
	if (!(sshStatusHandle =
		      RegisterServiceCtrlHandler(SVCNAME, service_ctrl)))
		return;
...
{ /* Handle the requested control code. */
	if (ctrl_code == SERVICE_CONTROL_STOP ||
	    ctrl_code == SERVICE_CONTROL_SHUTDOWN)
		service_stop();
	else
		send_status(ssStatus.dwCurrentState, NO_ERROR, 0,
			    1040 + ctrl_code);
} /* service_ctrl */
	...
	if (hMustTerminate) {
		...
		if (WaitForSingleObject(hThread, 4500) == WAIT_TIMEOUT) {
			if (hThread) {
				...
				event_log(
					1080,
					"Mixmaster Service terminated forcibly");
			}
		} else
...
static int set_stdfiles()
{ /* needed for _popen() */
	static DWORD std_handles[] = { STD_INPUT_HANDLE, STD_OUTPUT_HANDLE,
				       STD_ERROR_HANDLE };
	...
	for (stf_fileno = 0; stf_fileno <= 2; stf_fileno++) {
		...
		if (hStd == INVALID_HANDLE_VALUE)
			return 0;
		fh = _open_osfhandle((long)std_handles[stf_fileno],
				     (stf_fileno ? _O_WRONLY : _O_RDONLY) |
					     _O_BINARY);
		dup2(fh, stf_fileno);
			...
			DWORD id)
{
	...
	ssStatus.dwCheckPoint = ((current_state == SERVICE_RUNNING) ||
...
static int remove_service()
{
	...
	if (!(schService =
		      OpenService(schSCManager, SVCNAME, SERVICE_ALL_ACCESS))) {
		CloseServiceHandle(schSCManager);
...
static char *GetLastErrorText()
{
	...
	dwRet = FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER |
			      ...
			      NULL, err = GetLastError(), LANG_NEUTRAL,
			      (LPSTR)&lpszTemp, 0, NULL);
