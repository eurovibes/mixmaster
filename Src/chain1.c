...
int t1_rlist(REMAILER remailer[], int badchains[MAXREM][MAXREM])
{
	...
	while (fgets(line, sizeof(line), list) != NULL && n < MAXREM) {
		if (strleft(line, "$remailer") && strchr(line, '<') &&
		    strchr(line, '>') && strchr(line, '{') &&
		    strchr(line, '{') + 4 < strchr(line, '}')) {
		...
		if (strlen(line) >= 72 && strlen(line) <= 73)
			for (i = 1; i < n; i++)
				if (strleft(line, remailer[i].name) &&
					...
					remailer[i].info[1].latency =
						36000 * N(line[55]) +
						3600 * N(line[56]) +
						600 * N(line[58]) +
						60 * N(line[59]) +
						10 * N(line[61]) + N(line[62]);
					listed++;
				}
	}
	fclose(list);
	parse_badchains(badchains, TYPE1LIST, "Broken type-I remailer chains",
			remailer, n);
...
int t1_ek(BUFFER *key, BUFFER *seed, int num)
{
	...
	fprintf(stderr, "passphrase=%s (%2X%2X%2X%2X %d)\n", key->data,
		seed->data[0], seed->data[1], seed->data[2], seed->data[3],
		num);
#endif /* DEBUG */
	return (0);
}
	       ...
	       BUFFER *ek, BUFFER *feedback)
{
	...
	if (chain[0] == 0)
		chain[0] = chain_randfinal(type, remailer, badchains, maxrem, 1,
					   chain, chainlen, 0);
	...
	if (chain_rand(remailer, badchains, maxrem, chain, chainlen, 1, 0) ==
	    -1) {
	...
	for (hop = 0; hop < chainlen; hop++) {
		...
		if (remailer[chain[hop]].flags.pgp) {
			...
			err = pgp_encrypt(PGP_ENCRYPT | PGP_REMAIL | PGP_TEXT,
					  b, rem, NULL, NULL, NULL, NULL);
			if (err < 0) {
