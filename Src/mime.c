...
int decode_line(BUFFER *line)
{
	...
	for (i = 0; line->data[i] != '\0'; i++) {
		if (line->data[i] == '=') {
			if (isxdigit(line->data[i + 1]) &&
			    isxdigit(line->data[i + 2])) {
				c = hex(line->data[i + 1]) * 16 +
				    hex(line->data[i + 2]);
				i += 2;
				addprintable(out, c);
...
int decode_header(BUFFER *in)
{
	...
	for (in->ptr = 0; in->data[in->ptr] != '\0'; in->ptr++) {
		if (encoded == 'q' && in->data[in->ptr] == '=' &&
			...
			c = hex(in->data[in->ptr + 1]) * 16 +
			    hex(in->data[in->ptr + 2]);
			in->ptr += 2;
			addprintable(out, c);
		} else if (encoded == 'q' && in->data[in->ptr] == '_')
			buf_appendc(out, ' ');
		else if (in->data[in->ptr] == '=' &&
			 in->data[in->ptr + 1] == '?' &&
			 in->data[in->ptr + 2] != '\0') {
	...
	if (mboundary->length) {
		...
		if (bufleft(line, info_beginpgp)) {
			if (buffind(line, "(SIGNED)")) {
				...
				buf_appends(
					out,
					"[-- OpenPGP message with signature --]\n");
		...
		buf_rest(part, msg);
#ifdef USE_PGP
		if (pgp_decrypt(part, NULL, data, PGPPUBRING, NULL) ==
		    PGP_SIGOK)
			buf_appendf(out,
				    "[-- OpenPGP signature from:\n    %b --]\n",
				    data);
		else
...
int attachfile(BUFFER *message, BUFFER *filename)
{
	...
	if ((bufiright(filename, ".txt") || !bufifind(filename, ".")) &&
	    (strlen(DEFLTENTITY) != 0))
...
{
	BUFFER *out, *part, *line, *type, *subtype, *mboundary, *field,
		*content;
