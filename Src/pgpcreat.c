...
int pgp_packet(BUFFER *in, int type)
{
	...
	if (type > 15) {
      ...
      buf_appendc(out,  (in->length-192) & 0xFF);
		...
		ctb = 128 + (type << 2);
		if (in->length < 256 && type != PGP_PUBKEY &&
		    type != PGP_SECKEY && type != PGP_SIG &&
		    type != PGP_PUBSUBKEY && type != PGP_SECSUBKEY
#ifdef MIMIC
		    && type != PGP_ENCRYPTED
#endif /* MIMIC */
		) {
...
static int pgp_ideaencrypt(BUFFER *in, BUFFER *out, BUFFER *key, int mdc)
{
	...
	out->data[8 + mdc] = out->data[6 + mdc],
		      out->data[9 + mdc] = out->data[7 + mdc];
	if (mdc) {
		SHA1_Init(&c);
	...
	n = 0;
	idea_cfb64_encrypt(out->data + mdc, out->data + mdc, 10, &ks, iv, &n,
			   IDEA_ENCRYPT);
	if (!mdc) {
		iv[6] = iv[0], iv[7] = iv[1];
	...
	idea_cfb64_encrypt(in->data, out->data + 10 + mdc, in->length, &ks, iv,
			   &n, IDEA_ENCRYPT);
	if (mdc) {
		SHA1_Update(&c, "\xD3\x14", 2); /* 0xD3 = 0xC0 | PGP_MDC */
		idea_cfb64_encrypt("\xD3\x14", out->data + 11 + in->length, 2,
				   &ks, iv, &n, IDEA_ENCRYPT);
		SHA1_Final(out->data + 13 + in->length, &c);
		idea_cfb64_encrypt(out->data + 13 + in->length,
				   out->data + 13 + in->length, 20, &ks, iv, &n,
				   IDEA_ENCRYPT);
	}
	return (0);
}
...
static int pgp_castencrypt(BUFFER *in, BUFFER *out, BUFFER *key, int mdc)
{
	...
	n = 0;
	CAST_cfb64_encrypt(out->data + mdc, out->data + mdc, 10, &ks, iv, &n,
			   CAST_ENCRYPT);
	if (!mdc) {
		iv[6] = iv[0], iv[7] = iv[1];
		memcpy(iv, out->data + 2, 6);
	...
	CAST_cfb64_encrypt(in->data, out->data + 10 + mdc, in->length, &ks, iv,
			   &n, CAST_ENCRYPT);
	if (mdc) {
		SHA1_Update(&c, "\xD3\x14", 2); /* 0xD3 = 0xC0 | PGP_MDC */
...
static int pgp_bfencrypt(BUFFER *in, BUFFER *out, BUFFER *key, int mdc)
{
	...
	n = 0;
	BF_cfb64_encrypt(out->data + mdc, out->data + mdc, 10, &ks, iv, &n,
			 BF_ENCRYPT);
	if (!mdc) {
		iv[6] = iv[0], iv[7] = iv[1];
		memcpy(iv, out->data + 2, 6);
	...
	BF_cfb64_encrypt(in->data, out->data + 10 + mdc, in->length, &ks, iv,
			 &n, BF_ENCRYPT);
	if (mdc) {
		SHA1_Update(&c, "\xD3\x14", 2); /* 0xD3 = 0xC0 | PGP_MDC */
...
static int pgp_aesencrypt(BUFFER *in, BUFFER *out, BUFFER *key, int mdc)
{
	...
	out->data[16 + mdc] = out->data[14 + mdc],
		       out->data[17 + mdc] = out->data[15 + mdc];
	if (mdc) {
		SHA1_Init(&c);
	...
	n = 0;
	AES_cfb128_encrypt(out->data + mdc, out->data + mdc, 18, &ks, iv, &n,
			   AES_ENCRYPT);
	if (!mdc) {
	...
	AES_cfb128_encrypt(in->data, out->data + 18 + mdc, in->length, &ks, iv,
			   &n, AES_ENCRYPT);
	if (mdc) {
		SHA1_Update(&c, "\xD3\x14", 2); /* 0xD3 = 0xC0 | PGP_MDC */
		...
		AES_cfb128_encrypt(out->data + 21 + in->length,
				   out->data + 21 + in->length, 20, &ks, iv, &n,
				   AES_ENCRYPT);
	}
	return (0);
}
...
int pgp_symmetric(BUFFER *in, BUFFER *key, int mdc)
{
	...
	buf_prepare(out,
		    in->length + (mdc ? (1 + 2 + 22) : 2) + pgp_blocklen(sym));
	switch (sym) {
#ifdef USE_IDEA
	case PGP_K_IDEA:
		   ...
		   char *pubring)
{
	...
	sym = seskey->data[0];
	if ((algo = pgpdb_getkey(PK_ENCRYPT, PGP_ANY, &sym, NULL, NULL, key,
				 user, NULL, keyid, pubring, NULL)) == -1)
		goto end;
...
int pgp_signhashalgo(BUFFER *algo, BUFFER *userid, char *secring, BUFFER *pass)
{
	...
	pkalgo = pgpdb_getkey(PK_SIGN, PGP_ANY, NULL, NULL, NULL, NULL, userid,
			      NULL, NULL, secring, pass);
	if (pkalgo == PGP_S_DSA)
		buf_sets(algo, "sha1");
    ...
    secring:   key ring with signature key */
{
	...
	if (keypacket) {
		...
		algo = pgp_getkey(PK_SIGN, PGP_ANY, NULL, NULL, NULL, keypacket,
				  key, id, NULL, pass);
	} else
		algo = pgpdb_getkey(PK_SIGN, PGP_ANY, NULL, NULL, NULL, key,
				    userid, NULL, id, secring, pass);
	if (algo <= -1) {
		err = algo;
		goto end;
	}
	...
	case 4:
		...
		if (self) {
			...
			buf_setc(
				d,
				0x01); /* now we support MDC, so we can add MDC flag */
