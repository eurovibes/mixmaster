	...
	if ((keyring = mix_openfile(SECRING, "r")) == NULL) {
		...
		while (err == -1) {
			...
			if (strleft(line, begin_key)) {
				...
				created = 0;
				do {
					res = fgets(line, sizeof(line),
						    keyring);
					if (strileft(line, "created:")) {
						created = parse_yearmonthday(
							strchr(line, ':') + 1);
						if (created == -1)
							created = 0;
					} else if (strileft(line, "expires:")) {
						expires = parse_yearmonthday(
							strchr(line, ':') + 1);
						if (expires == -1)
							expires = 0;
					}
				...
				if (res == NULL)
					break;
				if (keyid &&
				    (strncmp(line, idstr, KEY_ID_LEN) != 0))
					continue;
				if (created != 0 && (created > time(NULL))) {
					errlog(ERRORMSG,
					       "Key is not valid yet (creation date in the future): %s",
					       idstr);
					break;
				}
				if (expires != 0 &&
				    (expires + KEYGRACEPERIOD < time(NULL))) {
					errlog(ERRORMSG, "Key is expired: %s",
					       idstr);
					break;
				}
...
int key(BUFFER *out)
{
	...
	if (MIX) {
		if (((f = mix_openfile("all_my_pubkeys.txt", "r")) != NULL) ||
			...
			buf_appends(
				out,
				"Use only one mixmaster key per remailer unless your client can handle multiple.\n");
			buf_appends(
				out,
				"A key length of '258' in this file means 1024-bit RSA as y=(x-2)*4.\n");
			buf_appends(
				out,
				"1024-bits offers inferior security to the larger keys that require at least version 3.0.2.\n");
			buf_appends(
				out,
	...
	if (err == -1 && UNENCRYPTED) {
		buf_appends(out,
			    "The remailer accepts unencrypted messages.\n");
		err = 0;
	}
	if (err == -1)
		...
		if (strleft(line, begin_key)) {
			...
			do {
				...
				if (strileft(line, "expires:")) {
					...
					if (expires == -1)
						expires =
							2147483647; /* no expiry date means far future */
					if ((expires + KEYGRACEPERIOD) <
					    time(NULL)) {
						show = 2; /* do not put this key in the new file - i.e. deletion */
						errlog(WARNING,
						       "Deleting expired key %s",
						       line);
					} else {
						show = 1; /* show this key */
					}
					if (1 == show)
						buf_write(header, newsecring);
					buf_free(header);
				}
		      ...
		      time_t created_found, time_t expires_found)
{
	...
	fprintf(out, "%s %s %s %s:%s %s%s", SHORTNAME, REMAILERADDR, line,
		mixmaster_protocol, VERSION, MIDDLEMAN ? "M" : "",
