 ...
 */
	...
	for (i = chain->length; i >= 0; i--)
		if (i == 0 || chain->data[i - 1] == ',' ||
			...
			for (j = i;
			     isspace(chain->data[j]);) /* ignore whitespace */
				j++;
			if (chain->data[j] == '\0')
				break;
...
start:
	...
	for (i = 1; i < maxrem; i++) {
		select[i] =
			((remailer[i].flags.mix &&
			  rtype == 0) || /* remailer supports type */
			 (remailer[i].flags.pgp && remailer[i].flags.ek &&
			  rtype == 1) ||
			 (remailer[i].flags.newnym && rtype == 2)) &&
			(remailer[i].info[t].reliability >= 100 * RELFINAL ||
			 constraints_ignored) && /* remailer has sufficient reliability */
			(remailer[i].info[t].latency <= MAXLAT ||
			 constraints_ignored) && /* remailer has low enough latency */
			(remailer[i].info[t].latency >= MINLAT ||
			 constraints_ignored) && /* remailer has high enough latency */
			(type == MSG_NULL ||
			 !remailer[i]
				  .flags
				  .middle) && /* remailer is not middleman */
			!remailer[i]
				 .flags
				 .star_ex && /* remailer is not excluded from random selection */
			(remailer[i].flags.post ||
			 type != MSG_POST) && /* remailer supports post when this is a post */
			((secondtolasthop == -1) ||
	...
	if (randavail == 0) {
		if (ignore_constraints_if_necessary && !constraints_ignored) {
			errlog(WARNING,
			       "No reliable remailers. Ignoring for randhop\n");
...
start:
	for (hop = 0; hop < chainlen; hop++)
		if (thischain[hop] == 0) {
			...
			err = 1;
			if (hop > 0)
				assert(thischain[hop -
						 1]); /* we already should have chosen a remailer after this one */
			for (i = 1; i < maxrem; i++) {
				select[i] =
					((remailer[i].flags.mix &&
					  t == 0) || /* remailer supports type */
					 (remailer[i].flags.pgp &&
					  remailer[i].flags.ek && t == 1)) &&
					(remailer[i].info[t].reliability >=
						 100 * MINREL ||
					 constraints_ignored) && /* remailer has sufficient reliability */
					(remailer[i].info[t].latency <=
						 MAXLAT ||
					 constraints_ignored) && /* remailer has low enough latency */
					(remailer[i].info[t].latency >=
						 MINLAT ||
					...
					!badchains[i]
						  [thischain[hop -
							     1]] && /* remailer can send to the next one */
					(hop == chainlen - 1 ||
			...
			for (i = hop - DISTANCE; i <= hop + DISTANCE; i++)
				if (i >= 0 && i < chainlen &&
				    select[thischain[i]] && thischain[i]) {
			...
			assert(randavail >= 0);
			if (randavail < 1) {
				if (ignore_constraints_if_necessary &&
				    !constraints_ignored) {
					errlog(WARNING,
					       "No reliable remailers. Ignoring for randhop\n");
					constraints_ignored = 1;
...
float chain_reliability(char *chain, int chaintype, char *reliability_string)
{
	...
	maxrem = (chaintype == 0 ? mix2_rlist(remailer, badchains) :
				   t1_rlist(remailer, badchains));
	...
	while (*name_end != '\0') { /* While string not scanned completely */
		...
		for (i = 0; (i < maxrem) &&
			    (strcmp(remailer[i].name, remailer_name) != 0);
		     i++)
			;
		if (!strcmp(remailer[i].name, remailer_name)) { /* Found it! */
			acc_reliability *=
				((float)remailer[i].info[chaintype].reliability) /
				10000;
			if (previous != -1) {
	...
	if (reliability_string) {
		if (acc_reliability < 0)
			...
			sprintf(reliability_string, "%6.2f",
				acc_reliability * 100);
